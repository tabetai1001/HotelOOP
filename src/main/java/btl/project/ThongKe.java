...
public class ThongKe {
    ...
    public void ShowHDDetails() throws IOException {
        ...
        if (phieuDVSsList.size() == 0) System.out.println("Người này không sử dụng dịch vụ");
        else {
    ...
    void PressHD_Details(ActionEvent event) throws SQLException, ClassNotFoundException, IOException {
        ...
        if (tbl_HoaDon.getSelectionModel().getSelectedItem() == null){
            JOptionPane.showMessageDialog(null,"Hãy chọn hóa đơn cần xem !");return;
        }
        ...
        db= new ConnectionDB();
        db.rs = db.stmt.executeQuery("SELECT * FROM phieudv pdv, DichVu dv, LoaiDV ldv WHERE pdv.MaDV = dv.MaDV and dv.MaLoaiDV  = ldv.MaLoaiDV ");
        ...
        for (int i=0;i<PDVList_Temp.size();i++){
            ...
            while (db.rs.next()){
                ...
                if (
                        ...
                        && tbl_HoaDon.getSelectionModel().getSelectedItem().getNgayInHD().equals(db.rs.getDate("Traphong"))

                )
                {

                    if (
                            (PDVList_Temp.get(i).getNgaySD().toLocalDate().isBefore(db.rs.getDate("Traphong").toLocalDate()) || PDVList_Temp.get(i).getNgaySD().toLocalDate().equals(db.rs.getDate("Traphong").toLocalDate()) )
                    && (PDVList_Temp.get(i).getNgaySD().toLocalDate().isAfter(db.rs.getDate("NgayDatPhong").toLocalDate()) || PDVList_Temp.get(i).getNgaySD().toLocalDate().equals(db.rs.getDate("NgayDatPhong").toLocalDate()) )
